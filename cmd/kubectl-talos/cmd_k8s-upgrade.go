...
func k8sUpgradeClusterCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "k8s-upgrade CLUSTER",
		Short: "Start a KubernetesUpgrade operation on a Talos cluster",
		Args:  cobra.ExactArgs(1),
	}
	cmd.Long = cmd.Short + "\n\nThis command will instruct the Talos operator to start a"
	cmd.Long += " KubernetesUpgrade operation on the given cluster."
	cmd.Long += "\nThis will upgrade Kubernetes nodes in the cluster in a rolling fashion,"
