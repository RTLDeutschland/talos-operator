...
func rollingRebootClusterCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rolling-reboot CLUSTER",
		Short: "Start a RollingReboot operation on a Talos cluster",
		Args:  cobra.ExactArgs(1),
	}
	cmd.Long = cmd.Short + "\n\nThis command will instruct the Talos operator to start a"
	cmd.Long += " RollingReboot operation on the given cluster."
	cmd.Long += "\nThis will reboot all nodes in the cluster in a rolling fashion,"
